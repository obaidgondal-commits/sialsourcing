...
This is a future deployment procedure, not authorization to deploy the current draft. First correct and verify the preservation issues in PRESERVATION-REVIEW.md. No deployment has been performed by the packaging script or GitHub validation workflow. This first release needs a staging rehearsal and a deliberate production cutover because the repository does not contain the live database, uploaded media or server-only configuration.
